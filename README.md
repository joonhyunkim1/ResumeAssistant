...
│   ├── maintenance.py # 시작 시 데이터 정리 (분해형 한글 → 완성형)
...
| 자료에 **⚠ 추출 품질** 배지와 '한글 자모가 분해된 형태(NFD)' 안내 | macOS 파일명이나 일부 PDF의 분해형 한글을 시작할 때 자동으로 완성형으로 정리했다는 뜻입니다. **재색인**을 누르면 검색 정확도가 회복되고 안내가 사라집니다 |
...
| 화면에 ⚠️ 오류 알림이 뜸 | 알림 내용과 함께 서버를 실행한 터미널의 로그(`WARNING`/`ERROR`)를 확인하세요. 오류는 조용히 넘어가지 않고 화면과 로그에 모두 남습니다 |
